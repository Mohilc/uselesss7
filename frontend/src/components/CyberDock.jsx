...
const CLIMATE_CHIPS = [
  { key: 'SUNNY', label: 'Sunny', emoji: '☀️', temp: 46, wifi: 96, connected: true, climate: 'SUNNY', color: 'text-amber-300 hover:border-amber-400' },
  { key: 'RAINY', label: 'Rainy', emoji: '🌧️', temp: 50, wifi: 22, connected: true, climate: 'RAINY', color: 'text-sky-300 hover:border-sky-400' },
  { key: 'STORM', label: 'Storm', emoji: '⚡', temp: 52, wifi: 0, connected: false, climate: 'STORM', color: 'text-purple-300 hover:border-purple-400' },
  { key: 'HOT', label: 'Inferno', emoji: '🔥', temp: 88, wifi: 75, connected: true, climate: 'HOT', color: 'text-rose-300 hover:border-rose-400' },
  { key: 'COLD', label: 'Blizzard', emoji: '❄️', temp: 32, wifi: 80, connected: true, climate: 'COLD', color: 'text-cyan-300 hover:border-cyan-400' },
];
...
const CyberDock = ({
  ...
  const [dockCategory, setDockCategory] = useState('climate');
  ...
  return (
    <div className="w-full flex flex-col items-center select-none relative z-30">
      ...
      <div className="glass-panel rounded-full px-3 py-2 sm:px-4 sm:py-2.5 flex flex-wrap items-center justify-between gap-2 sm:gap-4 shadow-2xl border border-white/15 max-w-4xl w-full backdrop-blur-xl">
        {/* Category Switcher & Preset Chips */}
        <div className="flex items-center gap-1.5 overflow-x-auto scrollbar-none py-0.5">
          {/* Dual Toggle Pill: Climate vs Mood */}
          <div className="flex items-center bg-black/40 rounded-full p-0.5 border border-white/10 shrink-0">
            <button
              onClick={() => setDockCategory('climate')}
              className={`px-2 py-0.5 rounded-full text-[10px] font-black uppercase tracking-wider transition-all flex items-center gap-1 ${
                dockCategory === 'climate'
          ...
          {/* Chips list */}
          {(dockCategory === 'climate' ? CLIMATE_CHIPS : QUICK_EMOTIONS).map((item) => {
            const isSelected = dockCategory === 'climate'
              ? currentClimate === item.climate
              : currentMoodKey === item.key;
            return (
              <button
                key={item.key}
                onClick={() => onApplyQuickMood(item)}
                className={`px-2.5 py-1 rounded-full text-xs font-semibold border transition-all flex items-center gap-1 shrink-0 active:scale-95 ${
                  isSelected
                    ? 'bg-gradient-to-r from-indigo-600 to-purple-600 border-white/50 text-white shadow-lg shadow-indigo-500/40 scale-105 ring-2 ring-indigo-400/40 font-bold'
                    : 'bg-white/5 border-white/10 text-slate-300 hover:bg-white/15 hover:border-white/25 hover:scale-105'
                } ${item.color}`}
                title={
                  dockCategory === 'climate'
                    ? `Switch screen weather to ${item.label} (${item.temp}°C, ${item.wifi}% Wi-Fi)`
                    : `Feel ${item.label} (${item.temp}°C, ${item.wifi}% Wi-Fi)`
                }
              >
          ...
          {/* If simulated, quick reset pill */}
          {isSimulated && (
            <button
              onClick={onResetHardware}
              className="ml-1 px-2 py-1 rounded-full text-[10px] font-bold border border-emerald-500/40 bg-emerald-500/15 text-emerald-300 hover:bg-emerald-500/25 transition-all shrink-0 flex items-center gap-1"
              title="Return to real live laptop hardware sensors"
            >
              <RotateCcw className="w-3 h-3" />
              <span className="hidden md:inline">Real Sensors</span>
            </button>
          )}
        </div>
